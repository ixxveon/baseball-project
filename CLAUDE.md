...
이 프로젝트는 스펙 주도 개발(spec-driven development)을 따르지 않는다. `docs/DOMAIN_SPEC.md`는 도메인 범위/담당자를 파악하기 위한 참고 문서일 뿐, 구현 전 승인이 필요한 고정 스펙이 아니다. 요구사항은 대화·PR 논의를 통해 유연하게 다뤄도 된다.

1. **Convention 확인** — `docs/CONVENTION.md` 기준으로 기존 패턴 파악
2. **기존 코드 패턴 확인** — 유사 도메인 코드를 먼저 읽고, 그 패턴을 우선 재사용
3. **도메인 범위 확인** — `docs/DOMAIN_SPEC.md`에서 담당 도메인과 대략적인 기능 범위 참고 (세부 구현은 담당자 재량)
...
- [ ] 브랜치명이 `feature/[도메인명]-[세부기능]` 형식인가? (도메인별 풀스택 오너십이라 영역(be/fe/ai) 구분 없음)
